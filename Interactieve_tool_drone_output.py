...
if analysis_type == "Orthomosaic":
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Start berekening"):
            # --- Schrijf bestand tijdelijk naar disk ---
            ...
            with rasterio.open(tmp_path) as src:
                ...
                if preview_mode:
                    ...
                    index = (2*G - R - B)/sumRGB if index_choice == "Excess Green (ExG)" else (1.4*R - G)/sumRGB
                    ...
                    for i, (ji, window) in enumerate(src.block_windows(1)):
                        ...
                        sumRGB = R + G + B
                        sumRGB = np.where(sumRGB == 0, 1e-6, sumRGB)
                        chunk = (2*G - R - B)/sumRGB if index_choice == "Excess Green (ExG)" else (1.4*R - G)/sumRGB
